...
class Header extends \ArrayObject {
	...
	public function send_headers(\Hanariu\Response $response = NULL, $replace = FALSE, $callback = NULL)
	{
		if ($response === NULL)
		{
			$response = \Hanariu\Request::initial()->response();
		}

		$protocol = $response->protocol();
		$status = $response->status();
		$processed_headers = array($protocol.' '.$status.' '.\Hanariu\Response::$messages[$status]);
		...
		{
			$processed_headers[] = 'Content-Type: '.\Hanariu\Hanariu::$content_type.'; charset='.\Hanariu\Hanariu::$charset;
		}

		if (\Hanariu\Hanariu::$expose AND ! isset($headers['x-powered-by']))
		{
			$processed_headers[] = 'X-Powered-By: '.\Hanariu\Hanariu::version();
		}
	...
	protected function _send_headers_to_php(array $headers, $replace)
	{
		...
		foreach ($headers as $key => $line)
		{
			if ($key == 'Set-Cookie' AND is_array($line))
			{
				...
				{
					\Hanariu\Cookie::set($name, $value['value'], $value['expiration']);
				}
