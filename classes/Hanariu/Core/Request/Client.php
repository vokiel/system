<?php namespace Hanariu\Core\Request;
...
abstract class Client {
	...
	public function execute(\Hanariu\Request $request)
	{
		...
		$orig_response = $response = \Hanariu\Response::factory();

		if (($cache = $this->cache()) instanceof \Hanariu\HTTP\Cache)
		...
		foreach ($this->header_callbacks() as $header => $callback)
		{
			if ($response->headers($header))
			{
				...
				elseif ($cb_result instanceof \Hanariu\Response)
				{
					$response = $cb_result;
				}
	...
	abstract public function execute_request(\Hanariu\Request $request, \Hanariu\Response $response);

	public function cache(\Hanariu\HTTP\Cache $cache = NULL)
	{
		if ($cache === NULL)
			return $this->_cache;
	...
	public static function on_header_location(\Hanariu\Request $request, \Hanariu\Response $response, \Hanariu\Client $client)
	{
		if ($client->follow() AND \in_array($response->status(), array(201, 301, 302, 303, 307)))
		{
			switch ($response->status())
			{
				...
				case 201:
				case 303:
					$follow_method = \Hanariu\Request::GET;
					break;
				case 302:
			...
			$follow_request = \Hanariu\Request::factory($response->headers('Location'))
			                         ->method($follow_method)
			                         ->headers(\Hanariu\Arr::extract($request->headers(), $client->follow_headers()));

			if ($follow_method !== \Hanariu\Request::GET)
			{
