...
class Handler {
	...
	public static function error_handler($code, $error, $file = NULL, $line = NULL)
	{
		...
		{
			throw new \Hanariu\ErrorException($error, $code, 0, $file, $line);
		}
		return TRUE;
	}
	...
	public static function shutdown_handler()
	{
		...
		try
		{
			...
			{
				\Hanariu\Cache::save('\Hanariu\Core\Filesystem::find_file()', \Hanariu\Hanariu::$_files);
			}
		}
		catch (\Exception $e)
		{
		...
		if (\Hanariu\Hanariu::$errors AND $error = \error_get_last() AND \in_array($error['type'], \Hanariu\Hanariu::$shutdown_errors))
		{
			...
			\Hanariu\Exception::handler(new \Hanariu\ErrorException($error['message'], $error['type'], 0, $error['file'], $error['line']));
			exit(1);
		}
	}
